...
public class AppSettings extends App {
	...
	private void account(List<EyeWidget> set, int width) {
		...
		int curAmount = device.getUser().inventory.countItem(device.getCurrency().getItem());
		number.setAllowed("[0-9]+");
		number.setLimit(3);
		number.setInput((curAmount > 127 ? 127 : curAmount) + "");
		...
		add.setAction(() -> {
			...
			if(input.matches("[0-9]+")) {
				...
				boolean s = device.addMoney(amount);
				this.refresh();
			}
		});
