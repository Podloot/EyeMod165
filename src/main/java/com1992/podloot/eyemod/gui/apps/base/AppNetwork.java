...
public class AppNetwork extends AppTextList {
	...
	public EyeWidget getMain(EyeBoxPanel box, int index, CompoundTag info) {
		...
		if(info.getString("sen").equals(device.getOwner()) || device.connect.getRouterData().owner.equals(device.getOwner())) {
			...
			delete.setAction(() -> {
				...
				pane.setAction(() -> {
					PacketHandler.INSTANCE.sendToServer(new ServerRouterData(device.connect.getRouter(), Action.REMOVE, "posts", IntTag.valueOf(getIndex(info))));
					this.refresh();
				});
				openPane(pane);
			});
	...
	public EyeWidget getExpendable(EyeBoxPanel box, int index, CompoundTag info) {
		...
		ListTag re = info.getList("re", Type.STRING.type);
		int h = 0;
	...
	public int getIndex(CompoundTag info) {
		ListTag psts = device.connect.getRouterData().data.getList("posts", Type.COMPOUND.type);
		for (int i = 0; i < psts.size(); i++) {
			CompoundTag p = psts.getCompound(i);
