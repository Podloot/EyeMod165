...
public class AppContacts extends App {
	...
	public boolean load() {
		...
		ListNBT cons = device.data.getList("contacts", Type.STRING);
		...
		add.setButton(new Line("text.eyemod.add"), getAppColor(), () -> {
			if (!add.getInput().isEmpty()) {
				if (device.hasData()) {
					device.data.addToList("contacts", StringNBT.valueOf(add.getInput()));
					this.refresh();
				}
			}
		});
	...
	public EyePanel getContactPane(int index, String name) {
		...
		rem.setAction(() -> {
			ListNBT cont = device.data.getList("contacts", Type.STRING);
			cont.remove(index);
