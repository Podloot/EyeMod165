...
public class AppPlayers extends App {
	...
	public boolean load() {
		...
		for(AbstractClientPlayerEntity p : device.getUser().clientLevel.players()) {
	...
	public EyeListPanel getPlayer(AbstractClientPlayerEntity p) {
		EyeListPanel lp = new EyeListPanel(players, 16);
		EyeLabel name = new EyeLabel(lp.getWidth()-18, 16, new Line(p.getScoreboardName()));
		name.setBack(appColor);
		lp.add(name, 18, 0);
		
		EyePlayer pl = new EyePlayer(16, 16, p.getScoreboardName());
		lp.add(pl, 0, 0);
		
		lp.setData(p.getScoreboardName());
