...
public class AppWeather extends App {
	...
	public EyePanel getBiomePanel() {
		...
		String name = device.getUser().level.getBiome(bp).unwrapKey().get().location().getPath();
		Holder<Biome> b = getBiomeKey();
		...
		Color c = new Color(b.value().getFogColor());
	...
	public String getRainChance() {
		float t = getBiomeKey().value().getDownfall();
		return (int)(t*100) + "%";
	}
	
	public String getHumidity() {
		float t = device.getUser().level.getRainLevel(0.5F);
		return (int)(t*100) + "%";
	}
	
	private Holder<Biome> getBiomeKey() {
		BlockPos bp = device.getUser().blockPosition();
		return device.getUser().level.getBiome(bp);
	}
	...
	public int getWaterColor(Holder<Biome> b) {
		if(device.getWorldID() == Dim.END.id) {
			...
			Color c = new Color(b.get().getWaterColor());
			return new Color(c.getRed(), c.getGreen(), c.getBlue()).getRGB();
		}
	}
	
	public int getGrassColor(Holder<Biome> b) {
		if(device.getWorldID() == Dim.OVERWORLD.id) {
			Color c = new Color(b.get().getFoliageColor());
			return new Color(c.getRed(), c.getGreen(), c.getBlue()).getRGB();
		} else if(device.getWorldID() == Dim.END.id) {
			return 0xffede99a;
		} else {
			return 0xff983d3d;
		}
